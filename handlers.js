// ============================================================
// handlers.js — All interaction & event handlers
// Reads shared state from global._bot set in index.js
// ============================================================
"use strict";
// ── Unpack everything from index.js ──────────────────────────
const {
client, db,
...
// ── dbLoaded is a mutable `let` — access via global._bot so
// mutations in index.js (dbLoaded = true) are visible here ───
Object.defineProperty(global, 'dbLoaded', {
get() { return global._bot._dbLoaded ?? false; },
set(v) { global._bot._dbLoaded = v; },
configurable: true,
});

// ── These are defined locally in handlers.js ─────────────────
// donutAPI, formatPlaytime, formatTimeLeft, formatEnchants,
// getPeriodCutoff, endGiveaway, endSplitOrStealGiveaway,
// recordAntiRaidAction, checkAntiRaid, buildGiveawayEmbed,
// buildDorkRow, buildGiveawayValueLeaderboard,
// buildVouchLeaderboard, buildPartnerLeaderboard, buildSoSEmbed
// ── Giveaway embed builders (moved here from index.js) ──────────
...
const keepBtn = new ButtonBuilder().setCustomId(`dork_keep_${dorkId}`).setLabel(" Keep").setStyle(ButtonStyle.Success);
const doubleLabel = forceDisableDouble ? " Double (N/A)" : ` Double (→ ${formatNumber(doubled)})`;
const doubleBtn = new ButtonBuilder().setCustomId(`dork_double_${dorkId}`).setLabel(doubleLabel).setStyle(ButtonStyle.Danger).setDisabled(!canDouble);
return new ActionRowBuilder().addComponents(keepBtn, doubleBtn);
}
function buildSoSEmbed(data) {
const endTimestamp = Math.floor(data.endsAt / 1000);
const prizeDisplay = typeof data.prize === "number" ? formatNumber(data.prize) : (data.prizeStr ?? data.prize);
return new EmbedBuilder()
.setColor(0xe67e22)
.setTitle(" GIVEAWAY SOS ")
.setDescription(
`**Prize: ${prizeDisplay}**\n\n` +
`Split = share equally | Steal = take all (if no one else steals)\n\n` +
` Ending: <t:${endTimestamp}:R>\n` +
` Host: <@${data.hostId}>\n` +
` Entries: **${(data.entries||[]).length}**`
...
// ── Ticket questions modal submit ─────────────────────────
// ── LOA modals ────────────────────────────────────────────
if (cid === "loa_request_modal") {
return handleLoaModalSubmit(interaction);
}
if (cid.startsWith("loa_accept_modal_")) {
const loaId = parseInt(cid.replace("loa_accept_modal_", ""));
return handleLoaAcceptModal(interaction, loaId);
}
if (cid.startsWith("loa_reject_modal_")) {
const loaId = parseInt(cid.replace("loa_reject_modal_", ""));
return handleLoaRejectModal(interaction, loaId);
}
if (cid.startsWith("ticket_q_")) {

// Parse type and isCustom from customId: ticket_q_custom_TypeName or ticket_q_default_TypeName
const withoutPrefix = cid.replace("ticket_q_", "");
const isCustom = withoutPrefix.startsWith("custom_");
const type = decodeURIComponent(withoutPrefix.replace(/^(custom|default)_/, ""));
// Collect answers
...
return handleTicketCreate(interaction, type, isCustom, answers);
}
if (cid === "message_modal") {
...
{ name: " Partner", value: "`/partnertracking`", inline: false },
{ name: " Weekly Payment", value: "`/weeklypayment` `/weeklypaymentpost`", inline: false },
...
if (commandName === "ban") {
const _permCheck = requirePerm(interaction, PermissionFlagsBits.BanMembers); if (_permCheck) return _permCheck;
...
const _permCheck = requirePerm(interaction, PermissionFlagsBits.ManageRoles); if (_permCheck) return _permCheck;
const target = interaction.options.getUser("user");
...
const _permCheck = requirePerm(interaction, PermissionFlagsBits.ManageChannels); if (_permCheck) return _permCheck;
...
.setPlaceholder("Enter description. Use :emoji_name: for emojis, **bold**, _italic_, etc.")
.setRequired(true)
.setMaxLength(4000)
),
new ActionRowBuilder().addComponents(
...
const _permCheck = requirePerm(interaction, PermissionFlagsBits.ManageMessages); if (_permCheck) return _permCheck;
...
try {
// Bot performs the rename — user doesn't need ManageChannels
await channel.setName(newName, `Ticket renamed by ${interaction.user.tag}`);
// Log stat — try to get ticket open time from channel creation
const openedAt = interaction.channel.createdTimestamp;
dbLogTicketStat(interaction.guildId, interaction.user.id, "rename", interaction.channelId, openedAt);
return interaction.reply({
embeds: [
new EmbedBuilder()
.setColor(0x1e40af)
...
// ==========================================================
if (commandName === "kick") {
const _permCheck = requirePerm(interaction, PermissionFlagsBits.KickMembers); if (_permCheck) return _permCheck;
const target = interaction.options.getUser("user");
const reason = interaction.options.getString("reason") ?? "No reason provided";
const member = await interaction.guild.members.fetch(target.id).catch(() => null);
...
const _permCheckLD = requirePerm(interaction, PermissionFlagsBits.ManageChannels); if (_permCheckLD) return _permCheckLD;
const founderId = process.env.FOUNDER_ID;
if (founderId && interaction.user.id !== founderId) {
return interaction.reply({ embeds: [errorEmbed("Only the founder can use this command.")], flags: MessageFlags.Ephemeral });
}
...
const embed = new EmbedBuilder()
.setColor(0x1e40af)
.setTitle(" DonutSMP Universe — Bot Commands")
.addFields(
{ name: " Moderation", value: "`/ban` `/unban` `/kick` `/timeout` `/untimeout` `/purge` `/slowmode` `/lockchannel` `/lockdown` `/unlockdown`", inline: false },
{ name: " Roles", value: "`/addrole` `/removerole` `/stafflist`", inline: false },
{ name: " Embeds", value: "`/embed` `/embedorganized`", inline: false },
...
{ name: " Tickets", value: "`/ticketpanelsend` `/close` `/ticketrename` `/ticketuseradd` `/ticketuserremove`", inline: false },
{ name: " Applications", value: "`/applicationpanelsend`", inline: false },
{ name: " Economy", value: "`/spawner` `/setspawnerprice` `/spawnerpricesend` `/paymenttracking`", inline: false },
{ name: " DonutSMP", value: "`/stats` `/lookup` `/ah` `/ah-recent` `/leaderboard`", inline: false },
{ name: " Partner", value: "`/partnertracking`", inline: false },
{ name: " Weekly Payment", value: "`/weeklypayment` `/weeklypaymentpost`", inline: false },
{ name: " Info", value: "`/serverinfo` `/userinfo` `/roleinfo` `/invitetracker` `/serverall`", inline: false },
{ name: " Pricing & Wait", value: "`/pricing` `/invite` `/pricingset` `/wait`", inline: false },

{ name: " Setup", value: "`/setup` `/setupview`", inline: false },
)
.setFooter({ text: `Problem? Contact ${founderMention}` })
.setTimestamp();
return interaction.reply({ embeds: [embed] });
}
// ── /wait ─────────────────────────────────────────────────
...
dbSaveLiveLeaderboards(interaction.guildId);
return;
}
// ── /stafflist ─────────────────────────────────────────────
if (commandName === "strike") return handleStrike(interaction);
if (commandName === "strikes") return handleViewStrikes(interaction);
if (commandName === "clearstrikes") return handleClearStrikes(interaction);
if (commandName === "stafflist") return handleStaffList(interaction);

// ── /paymenttracking ───────────────────────────────────────
if (commandName === "paymenttracking") {
if (!premiumGuilds.has(interaction.guildId)) {
return interaction.reply({ embeds: [new EmbedBuilder().setColor(0xf1c40f).setTitle("✦ Premium Required").setDescription("Payment tracking is a **premium feature**.\n\nDM <@1012989279049367592> to purchase premium for this server, then use `/premium key:<yourkey>` to activate it.").setTimestamp()], flags: MessageFlags.Ephemeral });
}
return handlePaymentTracking(interaction);
}
// ── /premium ──────────────────────────────────────────────
if (commandName === "premium") return handlePremiumCommand(interaction);
// ── /activationkey (owner only) ───────────────────────────
if (commandName === "activationkey") {
if (interaction.user.id !== "1012989279049367592") return interaction.reply({ embeds: [errorEmbed("You do not have permission to use this command.")], flags: MessageFlags.Ephemeral });
const key = generateActivationKey();
activationKeys.set(key, { used: false, createdAt: Date.now() });
await dbSaveActivationKey(key);
return interaction.reply({
embeds: [new EmbedBuilder().setColor(0xf1c40f).setTitle(" Activation Key Generated")
.setDescription("Send this key to the buyer. It can only be used **once**.")
.addFields({ name: "Key", value: `\`${key}\``, inline: false })
.setFooter({ text: "This key activates premium for one server" })
.setTimestamp()],
flags: MessageFlags.Ephemeral,
});
}
// ── /removepremium (owner only) ───────────────────────────
if (commandName === "removepremium") {
if (interaction.user.id !== "1012989279049367592") return interaction.reply({ embeds: [errorEmbed("You do not have permission to use this command.")], flags: MessageFlags.Ephemeral });
if (premiumGuilds.size === 0) return interaction.reply({ embeds: [errorEmbed("No servers have premium.")], flags: MessageFlags.Ephemeral });
const lines = [];
for (const [gId, data] of premiumGuilds.entries()) {
const g = client.guilds.cache.get(gId);
lines.push(`**${g?.name ?? gId}** (\`${gId}\`) — activated by <@${data.activatedBy}>`);
}
const buttons = [];
let i = 0;
for (const [gId] of premiumGuilds.entries()) {
if (i >= 5) break; // Discord max 5 buttons per row
buttons.push(new ButtonBuilder().setCustomId(`removeprem_${gId}`).setLabel(client.guilds.cache.get(gId)?.name?.slice(0,20) ?? gId).setStyle(ButtonStyle.Danger));
i++;
}
return interaction.reply({
embeds: [new EmbedBuilder().setColor(0xe74c3c).setTitle(" Remove Premium")
.setDescription("**Premium servers:**\n\n" + lines.join("\n"))
.setFooter({ text: "Click a button to remove premium from that server" })
...
const channelId = cfg.announceChannelId ?? cfg.welcomeChannelId ?? cfg.vouchChannelId;
...
const _p = requirePerm(interaction, PermissionFlagsBits.ManageMessages); if (_p) return _p;
const modal = new ModalBuilder().setCustomId("message_modal").setTitle("Send Message");
modal.addComponents(new ActionRowBuilder().addComponents(
new TextInputBuilder().setCustomId("message_text").setLabel("Message").setStyle(TextInputStyle.Paragraph).setRequired(true).setMaxLength(2000)
));
return interaction.showModal(modal);
}
// ── /weeklypayment ────────────────────────────────────────
if (commandName === "weeklypayment") return handleWeeklyPayment(interaction);
// ── /loa ─────────────────────────────────────────────────
if (commandName === "loa") return handleLoa(interaction);
if (commandName === "weeklypaymentpost") return handleWeeklyPaymentPost(interaction);
...
if (!premiumGuilds.has(interaction.guildId)) {
return interaction.reply({ embeds: [new EmbedBuilder().setColor(0xf1c40f).setTitle("✦ Premium Required").setDescription("Partner tracking is a **premium feature**.\n\nDM <@1012989279049367592> to purchase premium for this server, then use `/premium key:<yourkey>` to activate it.").setTimestamp()], flags: MessageFlags.Ephemeral });
}
if (!interaction.guild) return interaction.reply({embeds:[errorEmbed("Server only.")],flags:MessageFlags.Ephemeral});
const cfg=getGuildConfig(interaction.guildId);
...
" Member Role: " + (cfg.memberRoleId ? "<@&" + cfg.memberRoleId + ">" : "not set") + " (everyone non-staff has this)\n" +
" Strike Channel: " + (cfg.strikeChannelId ? "<#" + cfg.strikeChannelId + ">" : "not set") + " (where 3-strike alerts are sent)\n" +
" Promotion/Demotion Channel: " + (cfg.promotionChannelId ? "<#" + cfg.promotionChannelId + ">" : "not set")
)
...
if (cid === "setuproles_memberrole") {
const cfg=getGuildConfig(interaction.guildId); cfg.memberRoleId=interaction.values?.[0]??null; dbSaveGuildConfig(interaction.guildId);
return interaction.update({embeds:[new EmbedBuilder().setColor(0x2ecc71).setTitle(" Member Role Set").setDescription("Member role: "+(cfg.memberRoleId?"<@&"+cfg.memberRoleId+">":"none")).setTimestamp()],components:[]});
}
if (cid === "setuproles_strikechan") {
const cfg=getGuildConfig(interaction.guildId); cfg.strikeChannelId=interaction.values?.[0]??null; dbSaveGuildConfig(interaction.guildId);
return interaction.update({embeds:[new EmbedBuilder().setColor(0x2ecc71).setTitle(" Strike Channel Set").setDescription("Strike channel: "+(cfg.strikeChannelId?"<#"+cfg.strikeChannelId+">":"none")).setTimestamp()],components:[]});
}
if (cid === "setuproles_promodemotion") {
const cfg=getGuildConfig(interaction.guildId); cfg.promotionChannelId=interaction.values?.[0]??null; dbSaveGuildConfig(interaction.guildId);
return interaction.update({embeds:[new EmbedBuilder().setColor(0x2ecc71).setTitle(" Promo/Demotion Channel Set").setDescription("Channel: "+(cfg.promotionChannelId?"<#"+cfg.promotionChannelId+">":"none")).setTimestamp()],components:[]});
}
...
if (cid === "setuproles_loa") {
const cfg = getGuildConfig(interaction.guildId);
cfg.loaRoleId = interaction.values?.[0] ?? null;
dbSaveGuildConfig(interaction.guildId);
return interaction.update({
embeds: [new EmbedBuilder().setColor(0x2ecc71).setTitle(" LOA Role Set")
.setDescription("LOA Role: " + (cfg.loaRoleId ? "<@&" + cfg.loaRoleId + ">" : "cleared"))
.setTimestamp()],
components: [],
});
}
if (cid === "setupchannels_loa") {
if (!interaction.values?.length) return interaction.update({});
const cfg = getGuildConfig(interaction.guildId);
cfg.loaChannelId = interaction.values[0];
dbSaveGuildConfig(interaction.guildId);
return interaction.update({
embeds: [new EmbedBuilder().setColor(0x2ecc71).setTitle(" LOA Channel Set")
.setDescription("LOA requests will be sent to <#" + cfg.loaChannelId + ">")
.setTimestamp()],
components: [],
});
}
if (cid === "setuproles_loweststaff") {
const cfg = getGuildConfig(interaction.guildId);
cfg.lowestStaffRoleId = interaction.values?.[0] ?? null;
dbSaveGuildConfig(interaction.guildId);
return interaction.update({
embeds: [new EmbedBuilder()
.setColor(0x2ecc71)
.setTitle(" Lowest Staff Role Set")
.setDescription("Lowest Staff Role: " + (cfg.lowestStaffRoleId ? "<@&" + cfg.lowestStaffRoleId + ">" : "cleared") + "\n\n`/stafflist` will now show all roles at or above this role.")
.setTimestamp()],
components: [],
});
}
...
if (cid === "setupchannels_announce") {
if (!interaction.values?.length) return interaction.update({});
const cfg = getGuildConfig(interaction.guildId);
cfg.announceChannelId = interaction.values[0];
dbSaveGuildConfig(interaction.guildId);
return interaction.update({
embeds: [new EmbedBuilder().setColor(0x2ecc71).setTitle(" Announce Channel Set")
.setDescription(" Announcements will be sent to <#" + cfg.announceChannelId + ">").setTimestamp()],
components: [],
});
}
...
// Remove from active map and DB immediately
activeGiveaways.delete(messageId);
dbDeleteActiveGiveaway(messageId);
...
// ── Strike confirm/deny buttons ─────────────────────────────
if (customId.startsWith("strike_demote_yes_")) {
return handleStrikeDemoteYes(interaction, customId.replace("strike_demote_yes_",""));
}
if (customId.startsWith("strike_demote_no_")) {
return interaction.update({ embeds: [new EmbedBuilder().setColor(0x95a5a6).setTitle("Strike Action").setDescription("No action taken. User still has 3 strikes on record.").setTimestamp()], components: [] });
}
...
// ── LOA accept / reject buttons ────────────────────────────
if (customId.startsWith("loa_accept_") && !customId.includes("modal")) {
const loaId = parseInt(customId.replace("loa_accept_", ""));
return handleLoaAcceptButton(interaction, loaId);
}
if (customId.startsWith("loa_reject_") && !customId.includes("modal")) {
const loaId = parseInt(customId.replace("loa_reject_", ""));
return handleLoaRejectButton(interaction, loaId);
}

// ── Remove premium button (owner only) ────────────────────
if (customId.startsWith("removeprem_")) {
if (interaction.user.id !== BOT_OWNER_ID) return interaction.reply({ embeds: [errorEmbed("Owner only.")], flags: MessageFlags.Ephemeral });
const gId = customId.replace("removeprem_", "");
await dbRemovePremiumGuild(gId);
const gName = client.guilds.cache.get(gId)?.name ?? gId;
return interaction.update({
embeds: [new EmbedBuilder().setColor(0xe74c3c).setTitle("✦ Premium Removed").setDescription(`Premium removed from **${gName}**.`).setTimestamp()],
components: [],
});
}
...
return handleTicketButtonClick(interaction, t, false);
}
}
...
return handleTicketButtonClick(interaction, typeName, true);
}
// ── Ticket Close Button ───────────────────────────────────
...
// ── Ticket first-response stat tracking ─────────────────────
if (message.guild) {
const gCfg = getGuildConfig(message.guild.id);
const custPfx = (gCfg.ticketTypes ?? []).map(t => (t.prefix ?? t.name.toLowerCase().replace(/\s+/g,"-").replace(/[^a-z0-9-]/g,"")) + "-");
const allPfx = ["support-","giveaway-","spawner-","partnership-","member-report-","staff-report-","building-","mysterybox-",...custPfx];
if (allPfx.some(p => message.channel.name?.startsWith(p))) {
const staffRoleIds = [gCfg.staffRoleId, gCfg.helperRoleId, gCfg.pmRoleId, gCfg.ticketStaffRoleId].filter(Boolean);
const member = message.guild.members.cache.get(message.author.id);
const isStaff = member && staffRoleIds.some(r => member.roles.cache.has(r));
if (isStaff) {
const key = message.channel.id + "_responded";
if (!ticketResponseLogged.has(key)) {
ticketResponseLogged.add(key);
dbLogTicketStat(message.guild.id, message.author.id, "respond", message.channel.id, message.channel.createdTimestamp);
}
}
}
}
...
// ── Handler: ticket button click — show questions modal or create directly
async function handleTicketButtonClick(interaction, type, isCustom) {
const cfg = getGuildConfig(interaction.guildId);
// Get questions for this ticket type
let questions = [];
if (isCustom) {
const customType = cfg.ticketTypes?.find(t => t.name === type);
questions = customType?.questions ?? [];
}
// Default types have no questions unless configured
if (questions.length > 0) {
// Show a modal with up to 5 questions (Discord modal max 5 rows)
const modal = new ModalBuilder()
.setCustomId(`ticket_q_${isCustom ? "custom" : "default"}_${encodeURIComponent(type)}`)
.setTitle(`${type.slice(0,40)} — Questions`);
...
return handleTicketCreate(interaction, type, isCustom);
}
// ── Handler: create ticket channel ───────────────────────────
async function handleTicketCreate(interaction, type, isCustom = false, answers = []) {
const guild = interaction.guild;
const user = interaction.user;
const cfg = getGuildConfig(interaction.guildId);
let config;
...
// Build welcome embed — include question answers if provided
const welcomeEmbed = buildTicketWelcomeEmbed(type, user, guild, config.welcomeMsg ?? null);
if (answers.length > 0) {
welcomeEmbed.addFields(
{ name: "\u200b", value: "** Ticket Questions & Answers**", inline: false },
...answers.map(({ q, ans }) => ({ name: q.slice(0, 256), value: ans.slice(0, 1024) || "—", inline: false }))
);
}
...
// Close immediately in one tap — no modal needed
return handleTicketCloseModal(interaction, channelId, "Closed by staff");
}
...
// Log close stat
dbLogTicketStat(interaction.guildId, interaction.user.id, "close", channelId, channel.createdTimestamp);
...
// Restore active LOA timers from DB
try {
const loaRows = await dbLoadPendingLoas();
for (const row of loaRows) {
scheduleLoaRemoval(row.id, row.guild_id, row.user_id, row.loa_role_id, new Date(row.expires_at).getTime());
}

if (loaRows.length > 0) console.log(` Restored ${loaRows.length} active LOA timer(s)`);
} catch(e) { console.error("LOA restore error:", e.message); }
...
dbSaveActiveGiveaway(msg.id, interaction.channelId, interaction.guildId, data);
...
dbSaveActiveGiveaway(msg.id, msg.channelId ?? giveawayData.channelId, giveawayData.guildId, giveawayData);
...
const fn = interaction.replied || interaction.deferred ? "followUp" : "reply";
await interaction[fn]({ embeds, components, flags: MessageFlags.Ephemeral });
// Second message for Lowest Staff Role + LOA Role (Discord max 5 rows per message)
return interaction.followUp({
embeds: [new EmbedBuilder()

.setColor(0x5865f2)
.setTitle(" Extra Role Settings")
.setDescription(
"**Lowest Staff Role** — `/stafflist` shows this role and above.\n" +
"Current: " + (cfg.lowestStaffRoleId ? "<@&" + cfg.lowestStaffRoleId + ">" : "not set") + "\n\n" +
"**LOA Role** — added to staff when an LOA is approved, removed when it expires.\n" +
"Current: " + (cfg.loaRoleId ? "<@&" + cfg.loaRoleId + ">" : "not set")
)
.setTimestamp()],
components: [
new ActionRowBuilder().addComponents(
new RoleSelectMenuBuilder()
.setCustomId("setuproles_loweststaff")
.setPlaceholder(" Lowest Staff Role — /stafflist shows this and above")
.setMinValues(0).setMaxValues(1)
),
new ActionRowBuilder().addComponents(
new RoleSelectMenuBuilder()
.setCustomId("setuproles_loa")
.setPlaceholder(" LOA Role — given during approved leave of absence")
.setMinValues(0).setMaxValues(1)
),
],
flags: MessageFlags.Ephemeral,
...
" Announce: "+(cfg.announceChannelId?"<#"+cfg.announceChannelId+">":"not set")+"\n"+" Vouch: "+(cfg.vouchChannelId?"<#"+cfg.vouchChannelId+">":"not set")+"\n"+
...
new ActionRowBuilder().addComponents(new ChannelSelectMenuBuilder().setCustomId("setupchannels_announce").setPlaceholder(" Announcements channel").addChannelTypes(ChannelType.GuildText)),
...
new ActionRowBuilder().addComponents(new ChannelSelectMenuBuilder().setCustomId("setupchannels_loa").setPlaceholder(" LOA requests channel").addChannelTypes(ChannelType.GuildText)),
],
flags:MessageFlags.Ephemeral,
});
}
...
else if (section==="channels") { cfg.vouchChannelId=null; cfg.partnerChannelId=null; cfg.ticketLogsChannelId=null; cfg.raidWarningsChannelId=null; }
...
if (count < limit) return; // trigger AT the limit (count === limit triggers punishment)
const punishKey=guild.id+":"+userId;
if (Date.now()-(antiRaidPunished.get(punishKey)??0)<60000) return; // 60s cooldown per user per action
...
console.log(" Anti-raid timed out",userId,"in",guild.name,"for",actionType,"(count:"+count+"/limit:"+limit+")");
} else if (raidMember.permissions.has(PermissionFlagsBits.Administrator)) {
console.log(" Anti-raid: cannot timeout",userId,"— user is Administrator, skipping");
} else {
// Try kick as fallback if timeout is not possible
try {
await raidMember.kick("Anti-raid: exceeded "+actionType+" limit, could not timeout");
console.log(" Anti-raid: kicked",userId,"(could not timeout) in",guild.name);
} catch(kickErr) {
console.log(" Anti-raid: could not timeout or kick",userId,"—",kickErr.message);
}
}
} else {
...
// ============================================================
// In-memory active LOA map { loaId -> { guildId, userId, roleId, expiresAt, timeout } }
...
const isStaff = member && (staffRoleIds.some(r => member.roles.cache.has(r)) || member.permissions.has(PermissionFlagsBits.ManageGuild));
if (!isStaff) return interaction.reply({ embeds: [errorEmbed("Only staff can request an LOA.")], flags: MessageFlags.Ephemeral });
// Show modal asking for reason and duration
const modal = new ModalBuilder()
...
.setLabel("How long? (e.g. 7d, 14d, 30d)")
.setStyle(TextInputStyle.Short)
.setRequired(true)
.setMaxLength(10)
),
...
const cfg = getGuildConfig(interaction.guildId);
const durationStr = interaction.fields.getTextInputValue("loa_duration").trim().toLowerCase();
const reason = interaction.fields.getTextInputValue("loa_reason").trim();
// Parse duration — only days supported (e.g. 7d, 14d)
const daysMatch = durationStr.match(/^(\d+)d$/i);
if (!daysMatch) {
return interaction.reply({ embeds: [errorEmbed("Invalid duration format. Use days like `7d`, `14d`, `30d`.")], flags: MessageFlags.Ephemeral });
}
const days = parseInt(daysMatch[1]);
if (days < 1 || days > 365) {
return interaction.reply({ embeds: [errorEmbed("Duration must be between 1d and 365d.")], flags: MessageFlags.Ephemeral });
}
// Save to DB
const loaId = await dbSaveLoaRequest({
guildId: interaction.guildId,
userId: interaction.user.id,
reason,
days,
status: "pending",
loaRoleId: cfg.loaRoleId,
});
// Send to LOA channel for review
const channelId = cfg.loaChannelId;
if (!channelId) {
return interaction.reply({ embeds: [errorEmbed("No LOA channel set. Ask an admin to set one in `/setup → Channels`.")], flags: MessageFlags.Ephemeral });
}
const ch = interaction.guild.channels.cache.get(channelId);
if (!ch?.isTextBased()) {
return interaction.reply({ embeds: [errorEmbed("LOA channel not found or not a text channel.")], flags: MessageFlags.Ephemeral });
}
...
{ name: "Duration Requested", value: `**${days}d**`, inline: true },
{ name: "Reason", value: reason, inline: false },
)
.setFooter({ text: `LOA ID: ${loaId}` })
.setTimestamp();
const row = new ActionRowBuilder().addComponents(
new ButtonBuilder().setCustomId(`loa_accept_${loaId}`).setLabel(" Accept").setStyle(ButtonStyle.Success),
new ButtonBuilder().setCustomId(`loa_reject_${loaId}`).setLabel(" Reject").setStyle(ButtonStyle.Danger),
);
await ch.send({ embeds: [embed], components: [row] });
return interaction.reply({
embeds: [new EmbedBuilder().setColor(0x2ecc71).setTitle(" LOA Request Submitted")
.setDescription(`Your LOA request for **${days}d** has been sent for review.`)
.setTimestamp()],
flags: MessageFlags.Ephemeral,
});
}
async function handleLoaAcceptButton(interaction, loaId) {
// Show modal asking for accept reason and final duration
const modal = new ModalBuilder()
.setCustomId(`loa_accept_modal_${loaId}`)
.setTitle("Accept LOA");
modal.addComponents(
new ActionRowBuilder().addComponents(
new TextInputBuilder()
.setCustomId("acc_duration")
.setLabel("Approved duration (e.g. 7d, 14d)")
.setStyle(TextInputStyle.Short)
.setRequired(true)
.setMaxLength(10)
),
new ActionRowBuilder().addComponents(
new TextInputBuilder()
.setCustomId("acc_reason")
.setLabel("Message to staff member (optional)")
.setStyle(TextInputStyle.Paragraph)
.setRequired(false)
.setMaxLength(500)
...
async function handleLoaRejectButton(interaction, loaId) {
const modal = new ModalBuilder()
.setCustomId(`loa_reject_modal_${loaId}`)
.setTitle("Reject LOA");
modal.addComponents(
new ActionRowBuilder().addComponents(
new TextInputBuilder()
.setCustomId("rej_reason")
...
const days = parseInt(daysMatch[1]);
// Load LOA from DB
const res = await db.query("SELECT * FROM loa_requests WHERE id=$1", [loaId]).catch(() => ({ rows: [] }));
const loa = res.rows[0];
if (!loa) return interaction.reply({ embeds: [errorEmbed("LOA request not found.")], flags: MessageFlags.Ephemeral });
if (loa.status !== "pending") return interaction.reply({ embeds: [errorEmbed("This LOA has already been processed.")], flags: MessageFlags.Ephemeral });
const expiresAt = new Date(Date.now() + days * 86400000);
// Update DB
await db.query(
"UPDATE loa_requests SET status='approved', accepted_by=$2, accept_reason=$3, days=$4, expires_at=$5 WHERE id=$1",
[loaId, interaction.user.id, accReason, days, expiresAt]
).catch(e => console.error("LOA accept DB:", e.message));
// Add LOA role
const guild = interaction.guild;
const loaMember = await guild.members.fetch(loa.user_id).catch(() => null);

if (loaMember && loa.loa_role_id) {
await loaMember.roles.add(loa.loa_role_id, "LOA approved").catch(() => {});
}
// Schedule role removal
scheduleLoaRemoval(loaId, loa.guild_id, loa.user_id, loa.loa_role_id, expiresAt.getTime());
// DM the staff member
try {
const user = await interaction.client.users.fetch(loa.user_id).catch(() => null);
if (user) {
await user.send({
embeds: [new EmbedBuilder()
.setColor(0x2ecc71)
...
.addFields(
{ name: "Approved Duration", value: `**${days}d** (until <t:${Math.floor(expiresAt.getTime()/1000)}:D>)`, inline: false },
{ name: "Approved By", value: `<@${interaction.user.id}>`, inline: true },
...(accReason ? [{ name: "Note", value: accReason, inline: false }] : []),
)
.setTimestamp()],
});
}
...
.addFields(
{ name: "Staff Member", value: `<@${loa.user_id}>`, inline: true },
{ name: "Duration", value: `**${days}d**`, inline: true },
{ name: "Approved By", value: `<@${interaction.user.id}>`, inline: true },
{ name: "Expires", value: `<t:${Math.floor(expiresAt.getTime()/1000)}:D>`, inline: true },
...(accReason ? [{ name: "Note", value: accReason, inline: false }] : []),
)
.setTimestamp()],
components: [],
...
async function handleLoaRejectModal(interaction, loaId) {
const rejReason = interaction.fields.getTextInputValue("rej_reason").trim();
const res = await db.query("SELECT * FROM loa_requests WHERE id=$1", [loaId]).catch(() => ({ rows: [] }));
const loa = res.rows[0];
...
if (loa.status !== "pending") return interaction.reply({ embeds: [errorEmbed("This LOA has already been processed.")], flags: MessageFlags.Ephemeral });
await db.query(
"UPDATE loa_requests SET status='rejected', accepted_by=$2, reject_reason=$3 WHERE id=$1",
[loaId, interaction.user.id, rejReason]
).catch(e => console.error("LOA reject DB:", e.message));
// DM the staff member
try {
const user = await interaction.client.users.fetch(loa.user_id).catch(() => null);
if (user) {
...
embeds: [new EmbedBuilder()
.setColor(0xe74c3c)
.setTitle(" Your LOA Has Been Rejected")
.addFields(
{ name: "You requested", value: `**${loa.days}d**`, inline: true },
{ name: "Rejected By", value: `<@${interaction.user.id}>`, inline: true },
{ name: "Reason", value: rejReason, inline: false },
)
.setTimestamp()],
});
}
...
function scheduleLoaRemoval(loaId, guildId, userId, roleId, expiresAtMs) {
const delay = expiresAtMs - Date.now();
if (delay <= 0) {
removeLoaRole(loaId, guildId, userId, roleId);
return;
}
// For delays > 24h, re-check every 24h to avoid JS timer overflow

const safeDelay = Math.min(delay, 24 * 60 * 60 * 1000);
const timer = setTimeout(async () => {
if (Date.now() >= expiresAtMs) {
await removeLoaRole(loaId, guildId, userId, roleId);
} else {
scheduleLoaRemoval(loaId, guildId, userId, roleId, expiresAtMs);
}
}, safeDelay);
activeLoas.set(loaId, { guildId, userId, roleId, expiresAtMs, timer });
}
async function removeLoaRole(loaId, guildId, userId, roleId) {
activeLoas.delete(loaId);
if (!roleId) return;
try {
...
const member = await guild.members.fetch(userId).catch(() => null);
if (!member) return;
await member.roles.remove(roleId, "LOA expired").catch(() => {});
console.log(` LOA expired — removed role from ${userId} in ${guildId}`);
// Mark as completed in DB
await db.query("UPDATE loa_requests SET status='completed' WHERE id=$1", [loaId]).catch(() => {});
} catch (e) { console.error("LOA role removal error:", e.message); }
}
// ============================================================
// WEEKLY PAYMENT COMMAND
// ============================================================
async function handleWeeklyPayment(interaction) {
if (!interaction.guild) return interaction.reply({ embeds: [errorEmbed("Server only.")], flags: MessageFlags.Ephemeral });
const guildId = interaction.guildId;
// Clear option
const clearOpt = interaction.options.getString("clear");
if (clearOpt === "clear") {
await dbClearWeeklyPayments(guildId);
return interaction.reply({
embeds: [new EmbedBuilder().setColor(0xe74c3c).setTitle(" Weekly Payments Cleared")
.setDescription("All weekly payment records for this server have been cleared.")
.setTimestamp()],
flags: MessageFlags.Ephemeral,
});
}
...
const status = interaction.options.getString("status"); // "completed" | "not_completed"
const amountStr = interaction.options.getString("amount") ?? null;
if (!weeklyPaymentStore.has(guildId)) weeklyPaymentStore.set(guildId, new Map());
const gMap = weeklyPaymentStore.get(guildId);
gMap.set(target.id, { status, amount: amountStr, timestamp: Date.now() });
await dbSaveWeeklyPayment(guildId, target.id);
const emoji = status === "completed" ? " " : " ";
const label = status === "completed" ? "Completed" : "Not Completed";
const amountDisplay = (status === "completed" && amountStr) ? ` — **${amountStr}**` : "";
return interaction.reply({
embeds: [new EmbedBuilder()
.setColor(status === "completed" ? 0x2ecc71 : 0xe74c3c)
.setTitle(`${emoji} Weekly Payment Updated`)
.setDescription(`<@${target.id}> marked as **${label}**${amountDisplay}`)
.setFooter({ text: `Updated by ${interaction.user.username}` })
.setTimestamp()],
});
}
async function handleWeeklyPaymentPost(interaction) {
if (!interaction.guild) return interaction.reply({ embeds: [errorEmbed("Server only.")], flags: MessageFlags.Ephemeral });
const guildId = interaction.guildId;
const gMap = weeklyPaymentStore.get(guildId);
if (!gMap || gMap.size === 0) {
return interaction.reply({
embeds: [errorEmbed("No weekly payment records found. Use `/weeklypayment` to add users.")],
flags: MessageFlags.Ephemeral,
});
}
const completed = [];
...
completed.push(` <@${userId}>${amt}`);
} else {
notCompleted.push(` <@${userId}>`);
}
}

const allLines = [...completed, ...notCompleted];
const embed = new EmbedBuilder()
.setColor(0x5865f2)
.setTitle(" Weekly Payments")
.setDescription(allLines.join("\n") || "No records.")
.addFields(
{ name: " Completed", value: String(completed.length), inline: true },
{ name: " Not Completed", value: String(notCompleted.length), inline: true },
)
.setFooter({ text: `Posted by ${interaction.user.username}` })
.setTimestamp();
return interaction.reply({ embeds: [embed] });
...
const guildId = interaction.guildId;
// No key — just explain how to get it
if (!key) {
return interaction.reply({
embeds: [new EmbedBuilder()
.setColor(0xf1c40f)
.setTitle("✦ Premium — DonutSMP Universe Bot")
.setDescription(
"**To purchase premium, DM <@1012989279049367592>.**\n\n" +
"Once purchased, you'll receive a one-time activation key.\n" +
"Use `/premium key:<yourkey>` to activate it for this server.\n\n" +
"**Premium features include:**\n" +
" Partner Tracking\n" +
" Payment Tracking\n" +
" Staff Analytics\n" +
" Weekly Payment Board\n" +
" Ticket Close & Rename Tracking (Coming Soon)"
)
.setFooter({ text: "DM the owner to get your key" })
.setTimestamp()],
flags: MessageFlags.Ephemeral,
});
}
...
const keyData = activationKeys.get(normalised);
if (!keyData || keyData.used) {
return interaction.reply({
embeds: [errorEmbed("Invalid or already-used activation key. Please check the key and try again, or DM <@1012989279049367592>.")],
flags: MessageFlags.Ephemeral,
});
}
// Activate premium for this guild
keyData.used = true;
keyData.usedBy = guildId;
activationKeys.set(normalised, keyData);
premiumGuilds.set(guildId, { activatedBy: interaction.user.id, activatedAt: Date.now() });
await dbSavePremiumGuild(guildId, interaction.user.id);
await dbMarkKeyUsed(normalised, guildId);
return interaction.reply({
embeds: [new EmbedBuilder()
.setColor(0x2ecc71)
.setTitle("✦ Premium Activated!")
.setDescription(
"**This server now has premium!**\n\n" +
"All premium features are now unlocked:\n" +
" `/partnertracking`\n" +
" `/paymenttracking`\n" +
" Staff Analytics\n\n" +
"Thank you for supporting DonutSMP Universe Bot! "
)
.setTimestamp()],
});
}
...
const reason = interaction.options.getString("reason");
const cfg = getGuildConfig(interaction.guildId);
// Parse user IDs from the string (spaces or commas)
const userIds = [...new Set(rawUsers.split(/[,\s]+/).map(s => s.replace(/[<@!>]/g,'').trim()).filter(s => /^\d+$/.test(s)))];
if (!userIds.length) return interaction.reply({ embeds: [errorEmbed("No valid user IDs found. Separate multiple IDs with spaces or commas.")], flags: MessageFlags.Ephemeral });
await interaction.deferReply();
const results = [];
const threeStrikeUsers = [];
for (const uid of userIds) {
const key = interaction.guildId + ":" + uid;
const existing = strikeStore.get(key) ?? [];
const newStrike = { reason, by: interaction.user.id, timestamp: Date.now() };
existing.push(newStrike);
strikeStore.set(key, existing);
dbSaveStrike(interaction.guildId, uid, existing);
const count = existing.length;
results.push({ uid, count });
if (count >= 3) threeStrikeUsers.push({ uid, strikes: existing });
}
// Build result message
const plural = results.length > 1;
const mentions = results.map(r => `<@${r.uid}>`).join(" and ");

const strikeLines = results.map(r => `<@${r.uid}> strike ${r.count}/3`).join("\n");
const embed = new EmbedBuilder()
.setColor(0xe74c3c)
.setTitle(" Strike" + (plural ? "s" : "") + " Issued")
.setDescription(
mentions + (plural ? " were" : " was") + ` striked because "${reason}"
` + strikeLines
)
.setFooter({ text: "Issued by " + interaction.user.username })
.setTimestamp();
await interaction.editReply({ embeds: [embed] });
// Handle 3-strike users — send alert to strike channel
for (const { uid, strikes } of threeStrikeUsers) {
if (!cfg.strikeChannelId) continue;
try {
const ch = await client.channels.fetch(cfg.strikeChannelId).catch(() => null);
if (!ch) continue;
// Get member roles above member role
let member, roleList = "N/A";
try {
member = await interaction.guild.members.fetch(uid).catch(() => null);
if (member && cfg.memberRoleId) {
const memberRole = interaction.guild.roles.cache.get(cfg.memberRoleId);
if (memberRole) {
const staffRoles = member.roles.cache.filter(r => r.position > memberRole.position && r.id !== interaction.guild.id);
...
.setColor(0xff0000)
.setTitle(" User Reached 3 Strikes")
.setDescription(`<@${uid}> has reached **3/3 strikes**.`)
.addFields(
{ name: " User", value: `<@${uid}>`, inline: true },
{ name: " Staff Roles", value: roleList, inline: true },
{ name: " Strike History", value: strikeHistory, inline: false },
)
.setTimestamp();

const row = new ActionRowBuilder().addComponents(
new ButtonBuilder().setCustomId("strike_demote_yes_" + uid).setLabel(" Yes — Demote").setStyle(ButtonStyle.Danger),
new ButtonBuilder().setCustomId("strike_demote_no_" + uid).setLabel(" No — Keep Roles").setStyle(ButtonStyle.Secondary),
);
await ch.send({ embeds: [alertEmbed], components: [row] });
} catch(err) { console.error("Strike 3-alert error:", err.message); }
}
}
async function handleViewStrikes(interaction) {
const target = interaction.options.getUser("user");
const key = interaction.guildId + ":" + target.id;
const strikes = strikeStore.get(key) ?? [];
if (!strikes.length) {
return interaction.reply({ embeds: [new EmbedBuilder().setColor(0x2ecc71).setTitle("Strikes — " + target.username).setDescription("No strikes on record.").setTimestamp()], flags: MessageFlags.Ephemeral });
}
const lines = strikes.map((s,i) => `**Strike ${i+1}/3:** ${s.reason}
└ By <@${s.by}> • <t:${Math.floor(s.timestamp/1000)}:R>`);
return interaction.reply({
embeds: [new EmbedBuilder()
.setColor(0xe74c3c)
.setTitle("Strikes — " + target.username + " (" + strikes.length + "/3)")
.setDescription(lines.join("\n\n"))
.setTimestamp()],
flags: MessageFlags.Ephemeral,
});
}
async function handleClearStrikes(interaction) {
const target = interaction.options.getUser("user");
const key = interaction.guildId + ":" + target.id;
strikeStore.set(key, []);
dbSaveStrike(interaction.guildId, target.id, []);
return interaction.reply({
embeds: [new EmbedBuilder().setColor(0x2ecc71).setTitle(" Strikes Cleared").setDescription("All strikes for <@" + target.id + "> have been cleared.").setTimestamp()],
flags: MessageFlags.Ephemeral,
});
}
async function handleStrikeDemoteYes(interaction, userId) {
const cfg = getGuildConfig(interaction.guildId);
const guild = interaction.guild;
...
// Remove all roles above memberRole
let removedRoles = [];
if (cfg.memberRoleId) {
const memberRole = guild.roles.cache.get(cfg.memberRoleId);
if (memberRole) {
...
// No member role set — just log
return interaction.update({ embeds: [new EmbedBuilder().setColor(0xe74c3c).setTitle(" No Member Role Set").setDescription("Please set a Member Role in `/setup` → Roles to use automatic demotion.").setTimestamp()], components: [] });
}
// Log to promo/demotion channel
if (cfg.promotionChannelId) {
try {
const ch = await client.channels.fetch(cfg.promotionChannelId).catch(() => null);
if (ch) await ch.send({
embeds: [new EmbedBuilder()
.setColor(0xe74c3c)
.setTitle(" Staff Demotion — 3 Strikes")
.setDescription(`<@${userId}> was demoted by <@${interaction.user.id}> for reaching 3/3 strikes.`)
.addFields(
{ name: " User", value: `<@${userId}>`, inline: true },
{ name: " Demoted By", value: `<@${interaction.user.id}>`, inline: true },
{ name: " Roles Removed", value: removedRoles.length ? removedRoles.join(", ") : "None", inline: false },
)
.setTimestamp()],
}).catch(() => {});
} catch (_) {}
...
.setDescription(`<@${userId}> has been demoted. Roles removed: ${removedRoles.length ? removedRoles.join(", ") : "none"}.`)
.setTimestamp()],
components: [],
});
}